...
// Códigos de cobertura definidos pela especificação.
typedef enum
{
    AGUA = 0,
    SOLO_EXPOSTO = 1,
...
// Códigos de estado definidos pela especificação.
typedef enum
{
...
// Os nomes L, C, P, T, seed, LIMIAR, vento_linha, vento_coluna e V
// seguem os nomes usados explicitamente na especificação.
typedef struct
{
...
// Dados da floresta.
//
// ESTÁTICOS: cobertura, umidade e ativacao.
// DINÂMICOS: estado_atual/proximo_estado e tempo_atual/proximo_tempo.
//
...
int ler_focos_e_zonas(FILE *arquivo, const Configuracao *configuracao,
                             Foco **focos, int *F, Zona **zonas, int *Z)
{
    ...
    for (int i = 0; i < *F; i++)
    {
        if (fscanf(arquivo, "%d %d", &(*focos)[i].linha,
                   &(*focos)[i].coluna) != 2)
            return 0;
...
int calcular_potencial_ignicao(int linha, int coluna,
                                      const Configuracao *configuracao,
                                      const Floresta *floresta)
{
    int S = 0;

    // Somamos apenas as contribuições dos 8 vizinhos de Moore
    // que estão ESTADO_EM_CHAMAS no estado_atual.
    for (int k = 0; k < 8; k++)
    {
        ...
        int A = prop_linha * configuracao->vento_linha +
                prop_coluna * configuracao->vento_coluna;

        // A especificação exige aritmética inteira.
    ...
    int I = (S * fator_combustivel[floresta->cobertura[indice_celula]] *
             (100 - floresta->umidade[indice_celula])) / 100;

    return I;
}

void simular(const Configuracao *configuracao, Floresta *floresta,
                    Resultados *resultado)
{
    ...
    // Se não existe nenhuma célula em ESTADO_EM_CHAMAS após os focos,
    // a especificação determina que nenhum passo seja executado.
    int fogo_existe = 0;
    ...
    for (int p = 0; p < configuracao->P; p++)
    {
        // PASSO 1 — ativar zonas programadas para p.
        // A ativação modifica estado_atual antes do cálculo do próximo estado.
        for (long long i = 0; i < total_celulas; i++)
        {
            if (floresta->ativacao[i] == p && floresta->estado_atual[i] == ESTADO_INTACTA)
            {
                floresta->estado_atual[i] = ESTADO_CONTENCAO;
            }
        }

        // PASSO 2 — calcular proximo_estado e proximo_tempo exclusivamente
        // a partir de estado_atual/tempo_atual.
        ...
        for (int linha = 0; linha < configuracao->L; linha++)
        {
            ...
            {
                long long indice_celula = indice(configuracao, linha, coluna);
                int estado = floresta->estado_atual[indice_celula];

                // Por padrão, a célula continua com o mesmo estado/tempo.
                floresta->proximo_estado[indice_celula] = estado;
                floresta->proximo_tempo[indice_celula] = floresta->tempo_atual[indice_celula];
                ...
                {
                    int I = calcular_potencial_ignicao(linha, coluna,
                                                        configuracao, floresta);

                    if (I >= configuracao->LIMIAR)
                    {
                        floresta->proximo_estado[indice_celula] = ESTADO_EM_CHAMAS;
                        novas_ignicoes++;

                        if (floresta->cobertura[indice_celula] == VEG_RASTEIRA)
                            floresta->proximo_tempo[indice_celula] = 2;
                        else
                            floresta->proximo_tempo[indice_celula] = 4;
                    }
                    else
                    {
                ...
                {
                    int novo_tempo = floresta->tempo_atual[indice_celula] - 1;

                    if (novo_tempo == 0)
                    {
                        floresta->proximo_estado[indice_celula] = ESTADO_QUEIMADA;
                        floresta->proximo_tempo[indice_celula] = 0;
                    }
                    else
                    {
                        floresta->proximo_estado[indice_celula] = ESTADO_EM_CHAMAS;
                        floresta->proximo_tempo[indice_celula] = novo_tempo;
                    }
                }
        ...
        // > e não >= preserva o primeiro passo em caso de empate.
        ...
        // PASSO 4 — trocar estado_atual <-> proximo_estado
        // e tempo_atual <-> proximo_tempo.
        ...
        // PASSO 5 — verificar se ainda existe alguma célula em chamas.
        fogo_existe = 0;
...
void calcular_resultados(const Configuracao *configuracao,
                                const Floresta *floresta,
                                Resultados *resultado)
{
    long long total_celulas = (long long)configuracao->L * configuracao->C;
    ...
    else
    {
        ...
        resultado->percentual_protegido =
            100.0 * resultado->contencao /
            combustiveis_iniciais;
    }
}

unsigned long long calcular_checksum(const Configuracao *configuracao,
                                            const Floresta *floresta)
{
    long long total_celulas = (long long)configuracao->L * configuracao->C;
    ...
    for (long long i = 0; i < total_celulas; i++)
    {
        checksum = checksum * 31ULL +
                   (unsigned long long)floresta->estado_atual[i];

        checksum = checksum * 31ULL +
                   (unsigned long long)floresta->tempo_atual[i];
    }
...
int main(int argc, char *argv[])
{
    if (argc != 2)
    {
        fprintf(stderr, "Uso: ./fire_seq entrada.txt\n");
        return 1;
    }
